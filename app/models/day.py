...
days_tags = Table(
    ...
    ForeignKeyConstraint(["day_timestamp", "user_id"], ["days.timestamp", "days.user_id"]),
...
class Day(Base, TimestampWithUpdateMixin):
    ...
    ai_generated_at: Mapped[dt.datetime | None] = mapped_column(
    ...
    trackable_progresses: Mapped[list["TrackableProgress"]] = relationship(
        back_populates="day", overlaps="user,trackable_progresses"
    )
    insights: Mapped[list["Insight"]] = relationship(back_populates="day", overlaps="user,insights")
    suggestions: Mapped[list["Suggestion"]] = relationship(
        back_populates="day", overlaps="user,suggestions"
    )
